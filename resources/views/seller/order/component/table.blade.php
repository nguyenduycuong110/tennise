    ...
    <tbody>
        @if(isset($orders) && is_object($orders))
            ...
            <tr >
                ...
                <td>
                    <a href="{{ route('seller.order.detail', $order->id) }}">{{ $order->code }}</a>
                </td>
                <td>
                ...
                <td class="text-right order-total">
                    @php
                        ...
                        foreach ($order->products as $product) {
                            if($product->seller_id != $seller->id) continue;
                            $sellerTotal += $product->pivot->price * $product->pivot->qty;
                        }
                    @endphp
                    {{ convert_price($sellerTotal + $order->shipping, true) }}
                </td>
